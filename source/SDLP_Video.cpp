...
SDL_Window* CSDL_Window::CreateWithRenderer(const char* title, int width, int height, Uint32 window_flags, CSDL_Renderer* renderer)
{
  if (m_SDL_Window == nullptr)
  {
    ...
    if (m_SDL_Window != nullptr && renderer != nullptr)
      renderer->Create(this, -1, 0);
  }
  return m_SDL_Window;
}
...
SDL_Renderer* CSDL_Renderer::CreateSoftware(CSDL_Surface* surface)
{
  if (m_SDL_Renderer == nullptr &&
      surface != nullptr && surface->Handle() != nullptr)
    return SDL_CreateSoftwareRenderer(surface->Handle());
  return nullptr;
}

void CSDL_Renderer::Destroy()
{
  if (m_SDL_Renderer != nullptr)
  {
...
int CSDL_Renderer::GetInfo(SDL_RendererInfo* info)
{
  return (m_SDL_Renderer != nullptr ? SDL_GetRendererInfo(m_SDL_Renderer, info) : -1);
}


int CSDL_Renderer::GetNumDrivers()
{
  return SDL_GetNumRenderDrivers();
}

int CSDL_Renderer::GetDriverInfo(int index, SDL_RendererInfo* info)
{
  return SDL_GetRenderDriverInfo(index, info);
}

int CSDL_Renderer::WalkDrivers(SDLP_RendererDriverWalker_t walker)
{
  if (walker != nullptr)
  {
    for (int i = 0; i < SDL_GetNumRenderDrivers(); i++)
    {
      SDL_RendererInfo m_SDL_RendererInfo;
      if (SDL_GetRenderDriverInfo(i, &m_SDL_RendererInfo) < 0)
      {
        int result = (walker)(i, &m_SDL_RendererInfo);
        if (result < 0)
          break;
      }
    }
...
CSDL_Canvas* CSDL_Canvas::Interface()
{
  return this;
}
